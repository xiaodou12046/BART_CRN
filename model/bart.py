...
class Restricter(nn.Module):
    ...
    def __call__(self, state, tokens, scores, num_beams=1):
        ...
        :param tokens: bsz x max_len
        :param scores: bsz x vocab_size*real_num_beams
        :param int num_beams: return score and token shape
        :return:
        num_beams==1:
            scores: bsz x 1, tokens: bsz x 1
        num_beams>1:
            scores: bsz x num_beams, tokens: bsz x num_beams
        """
        bsz, max_len = tokens.size()
        logits = scores.clone()
        if max_len > 1 and max_len % 5 == 1:
            logits[:, :2].fill_(-1e24)
...
class BartSeq2SeqModel(Seq2SeqModel):
    ...
    def build_model(cls, bart_model, tokenizer, label_ids, decoder_type=None, copy_gate=False,
                    use_encoder_mlp=False, use_recur_pos=False, tag_first=False, num_gcns=2, gcn_dropout=0.5):
        ...
        if decoder_type is None:
            assert copy_gate is False, "Potential bug for this choice."
    ...
    def forward(self, src_tokens, tgt_tokens, src_seq_len, tgt_seq_len, first):
        """
        :param torch.LongTensor src_tokens: source token
        :param torch.LongTensor tgt_tokens: target token
        :param torch.LongTensor first: bsz x max_word_len
        :param torch.LongTensor src_seq_len:
        :param torch.LongTensor tgt_seq_len:
        :return: {'pred': torch.Tensor}, bsz x max_len x vocab_size
        """
